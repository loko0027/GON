...
export default function GoleirosTab() {
  ...
  // ✅ 1. FUNÇÃO SEGURA PARA FORMATAR A DATA E EVITAR BUG DE FUSO HORÁRIO
  ...
  const handleSubmitConvocacao = () => {
    ...
    // Validação final do intervalo antes de enviar
  ...
  // ✅ 2. FUNÇÃO ADICIONADA DE VOLTA PARA EXIBIR A DURAÇÃO
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView style={styles.form}>
          ...
          {/* ✅ 2. CHAMADA PARA A FUNÇÃO DE INFORMAÇÃO DO INTERVALO ADICIONADA AQUI */}
          {renderInfoIntervalo()}
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.goleirosList} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refreshGoleiros} tintColor="#10B981" colors={['#10B981']}/>}>
        {filteredGoleiros.map((goleiro) => {
          ...
          return (
            <View key={goleiro.id} style={styles.goleiroCard}><View style={styles.goleiroHeader}><View style={styles.goleiroInfo}><View style={styles.goleiroAvatar}>{goleiro.foto_url ? <Image source={{ uri: goleiro.foto_url }} style={styles.avatarImage} /> : <User size={24} color="#10B981" />}</View><View><Text style={styles.goleiroNome}>{goleiro.nome}</Text><View style={styles.goleiroStats}><View style={styles.statItem}><Star size={14} color="#F59E0B" /><Text style={styles.statText}>{goleiro.nota_media?.toFixed(1) ?? '0.0'}</Text></View><View style={styles.statItem}><Target size={14} color="#6b7280" /><Text style={styles.statText}>{goleiro.jogos_realizados} jogos</Text></View></View></View></View><TouchableOpacity style={styles.convocarButton} onPress={() => handleConvocar(goleiro)}><Plus size={16} color="#fff" /><Text style={styles.convocarText}>Convocar</Text></TouchableOpacity></View><View style={styles.goleiroTags}>{goleiro.tags.map((tag: string, index: number) => (<View key={index} style={styles.tag}><Text style={styles.tagText}>{tag}</Text></View>))}</View></View>
          );
        })}
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  // ✅ ESTILOS ADICIONADOS PARA A INFORMAÇÃO DO INTERVALO
  intervaloInfo: {
    padding: 8,
