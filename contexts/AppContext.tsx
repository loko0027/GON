...
// ===== INTERFACE COMPLETA DO CONTEXTO (AppContextType) =====
...
  // --- Lógica de Cálculo de Valores da Convocação ---
...
  // --- Funções de Load e Setup (omitidas para brevidade, mas incluídas no arquivo completo) ---
  // Funções como fetchConvocacoes, loadSaldos, loadCategorias, loadRecargas, loadSaques, 
  // loadChamadosSuporte, loadMensagensSuporte, loadAllUsers, loadUserData, loadData, 
  // handleRefresh, clearAllAppDataStates, e useEffects de setup estão aqui.

  // INÍCIO DAS FUNÇÕES DE LOAD (Para que o arquivo seja completo)
...
      await loadData();
      Alert.alert('Sucesso', 'Solicitação de saque enviada para aprovação! O valor foi deduzido do seu saldo.');
    } catch (error: any) {
...
      if (updateError) throw updateError;
      await loadSaques();
      Alert.alert('Sucesso', 'Saque aprovado com sucesso! (Transferência PIX concluída)');
    } catch (error: any) {
...
      }
      await loadData();
      Alert.alert('Sucesso', 'Saque rejeitado e valor devolvido ao saldo do usuário!');
    } catch (error: any) {
...
      
      await loadData();
      Alert.alert('Sucesso', `Transferência de ${formatarMoeda(valor)} realizada com sucesso!`);
    } catch (error: any) {
      console.error("Erro ao transferir coins:", error);
      Alert.alert('Erro', error.message || 'Não foi possível realizar a transferência');
    }
  }, [user, getSaldoUsuario, getSaldo, loadData, formatarMoeda]);
...
    }
  }, [user, getSaldoUsuario, loadData, formatarMoeda]);
...
      }
      await loadData();
      Alert.alert('Sucesso', 'Convocação recusada com sucesso! O valor foi devolvido ao saldo do organizador.');
    } catch (error: any) {
...
      await loadData();
      Alert.alert('Sucesso', 'Goleiro avaliado com sucesso! Valor liberado para o goleiro.');
    } catch (error: any) {
...
    try {
      if (!user) throw new Error('Usuário não autenticado');
      const { error } = await supabase.from('chamados_suporte').insert({ ...chamado, solicitante_id: user.id, status: 'aberto' });
      if (error) throw error;
      await loadChamadosSuporte();
      Alert.alert('Sucesso', 'Chamado de suporte criado com sucesso! Em breve, nossa equipe entrará em contato.');
    } catch (error: any) {
      console.error("Erro ao criar chamado de suporte:", error);
      Alert.alert('Erro', error.message || 'Não foi possível criar o chamado de suporte');
    }
  }, [user, loadChamadosSuporte]);

  const aprovarChamadoSuporte = useCallback(async (chamadoId: string): Promise<void> => {
    try {
      const { error } = await supabase.from('chamados_suporte').update({ status: 'em_progresso' }).eq('id', chamadoId);
      if (error) throw error;
      await loadChamadosSuporte();
      Alert.alert('Sucesso', 'Chamado aprovado e em progresso!');
    } catch (error: any) {
...
      if (error) throw error;
      await loadChamadosSuporte();
      Alert.alert('Sucesso', 'Chamado recusado. Não haverá acompanhamento.');
    } catch (error: any) {
...
      Alert.alert('Mensagem Enviada', 'Sua mensagem foi enviada com sucesso para o chamado.');
    } catch (error: any) {
...
    return chamadosSuporte.filter(c => c.solicitante_id === userId).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  }, [chamadosSuporte]);

  const getMensagensPorChamado = useCallback((chamadoId: string): MensagemSuporte[] => {
    return mensagensSuporte.filter(m => m.chamado_id === chamadoId).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
  }, [mensagensSuporte]);

  const getChamadosAbertos = useCallback((): ChamadoSuporte[] => {
    return chamadosSuporte.filter(c => c.status === 'aberto' || c.status === 'em_progresso');
  }, [chamadosSuporte]);
...
    const convocacao = convocacoes.find(c => c.id === convocacaoId);
    return convocacao?.avaliado_goleiro === true;
  }, [convocacoes]);

  const isOrganizadorAvaliado = useCallback((convocacaoId: string): boolean => {
    const convocacao = convocacoes.find(c => c.id === convocacaoId);
    return convocacao?.avaliado_organizador === true;
  }, [convocacoes]);
...
    formatarData,
    formatarMoeda,
    calcularDetalhesConvocacao,
  }), [
    convocacoes, saldos, categorias, recargas, saques, chamadosSuporte, mensagensSuporte, allAppUsers, refreshing, loading,
    getAllUsers, getUsuariosPendentes, aprovarUsuario, rejeitarUsuario, getUsuarioById, atualizarUsuario,
    getSaquesPendentes, getRecargasPendentes, getTransacoesUsuario,
    fetchConvocacoes, criarConvocacao, aceitarConvocacao, recusarConvocacao, avaliarGoleiro, avaliarOrganizador, confirmarPresenca,
    getConvocacoesPorUsuario, getConvocacoesAtivas, getConvocacoesHistorico,
    recarregarCoins, aprovarRecarga, rejeitarRecarga, solicitarSaque, aprovarSaque, rejeitarSaque, getSaldo, getSaldoUsuario, transferirCoins,
    criarChamadoSuporte, aprovarChamadoSuporte, recusarChamadoSuporte, resolverChamadoSuporte, enviarMensagemSuporte, getChamadosPorUsuario,
    getMensagensPorChamado, getChamadosAbertos, getChamadosResolvidos,
    loadData, loadChamadosSuporte, handleRefresh, loadUserData,
    isGoleiroAvaliado, isOrganizadorAvaliado, formatarData, formatarMoeda, calcularDetalhesConvocacao,
  ]);
